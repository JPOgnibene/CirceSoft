...
Behavior:
  ...
  - Continuous feet math (no rounding of current position).
  - Tracks:
        cableRemaining_ft (starts 300 ft; decremented by actual feet moved),
        distanceTraveled_ft (cumulative),
        distanceRemaining_ft (from current fractional position),
        percentBatteryRemaining (−1% per waypoint).
  - PUTs to /current-values ONLY on:
        • START heartbeat,
...
CABLE_MAX_FT = 300.0
EPS_FT       = 1e-6             # tolerance for arrival tests
...
# ===== Continuous distance math (matches planner metric) =====
...
def grid_to_xy_feet(r: float, c: float) -> dict:
...
class Simulator:
    ...
    def reset_state(self) -> None:
        """
        Reset all local runtime variables to defaults.

        IMPORTANT:
        - This function does NOT make any PUT requests.
        - It is purely internal so the frontend sees no change when a reset happens.
        """
        self.path: list[dict] = []
        self.curr = {"r": 0.0, "c": 0.0}      # current fractional position
        self.next_idx = 0                     # index into path of next waypoint
        self.running = False                  # /directions == "START"
        ...
        print(f"[{iso_now()}] Simulator state reset.\n")
    ...
    async def get_directions(self, session: aiohttp.ClientSession) -> str:
        try:
    ...
    async def put_directions_stop(self, session: aiohttp.ClientSession) -> None:
        """
        Write {"command":"STOP"} to /directions.
        This will usually trigger the backend's emergency-stop broadcast.
        """
        payload = {"command": "STOP"}
        print(f"[{iso_now()}] PUT /directions payload:\n{json.dumps(payload, indent=2)}")
        try:
            async with session.put(HTTP_DIR_URL, json=payload) as r:
                if not (200 <= r.status < 300):
                    txt = await r.text()
                    print(f"[{iso_now()}] PUT /directions failed {r.status}: {txt}")
        except Exception as e:
            print(f"[{iso_now()}] PUT /directions error: {e}")

    # ---------- /grid/path ----------
    async def load_path_if_changed(self, session: aiohttp.ClientSession) -> None:
    ...
    def distance_remaining_ft(self) -> float:
        """Feet from current position to end, using the continuous metric."""
    ...
    async def put_status(
        ...
        *,
        waypoint_number_1based: int | None = None,
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
                ...
                while True:
                    ...
                    if dir_value == "START" and has_path:
                        ...
                        if first_target:
                            print(
                                f"[{iso_now()}] START received. "
                                f"First target: r={int(first_target['r'])}, c={int(first_target['c'])}"
                            )
                        else:
                            print(f"[{iso_now()}] START received but path has a single point.")
                        await self.put_status(
                            session,
                            waypoint_number_1based=None,
                            is_moving=True,
                            note="START received; beginning movement",
                        )
                        break
                ...
                while True:
                    ...
                    # If route finished, send STOP to /directions, reset state, and go back to WAIT phase
                    if self.path and self.next_idx >= len(self.path):
                        print(f"[{iso_now()}] Route complete — sending STOP to /directions and resetting state.")
                        await self.put_directions_stop(session)
                        self.reset_state()
                        break

                    # Idle if not running or no path yet
                    ...
                    if remaining_ft <= EPS_FT or step_ft >= remaining_ft - EPS_FT:
                        ...
                        # If that was the final waypoint, the next loop iteration
                        # will hit the completion block above and send STOP/reset.
                    else:
                        # Partial progress along geodesic
